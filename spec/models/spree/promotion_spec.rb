...
describe Spree::Promotion, type: :model do
  ...
  describe '#activate' do
    let(:promotion) { create(:multicode_promotion) }
  ...
  context '#usage_limit_exceeded?' do
    ...
    context 'there is a usage limit set' do
      let!(:promotion) { create(:multicode_promotion, :with_order_adjustment, usage_limit: usage_limit) }
  ...
  context '#usage_count' do
    let!(:promotion) { create(:multicode_promotion, name: 'Foo', code: 'XXX') }
  ...
  context '#eligible?' do
    ...
    context "when the promotion code's usage limit is exceeded" do
      let(:adjustable) { create(:order) }
      let(:promotion) { create(:multicode_promotion, :with_order_adjustment, code: 'abc123', per_code_usage_limit: 1) }
  ...
  describe '#line_item_actionable?' do
    ...
    context 'when the order is not eligible for the promotion' do
      ...
      context 'due to promotion code not being eligible' do
        let(:order) { create(:order) }
        let(:promotion) { create(:multicode_promotion, per_code_usage_limit: 0) }
  ...
  describe 'adding items to the cart' do
    ...
    let(:promo) { create :multicode_promotion_with_item_adjustment, adjustment_rate: 5, code: 'promo' }
