...
describe Spree::PromotionCode do
  ...
  context '#usage_limit_exceeded?' do
    ...
    let(:promotion) { create(:multicode_promotion, :with_order_adjustment, per_code_usage_limit: per_code_usage_limit) }
  ...
  context '#usage_count' do
    let(:promotable) { create(:order) }
    let(:promotion) { create(:multicode_promotion, :with_order_adjustment, code: 'abc123') }
